...
metrics = ['$', 'H20', 'TOI', '^', 'CF', 'DRT', 'POP', 'LS', 'AG']
...
def loss_batch(model, criterion, xb, yb, opt=None):
    ...
    pop_density_acc  = accuracy(pop_density, yb['pop_density'], topk=(1,1))
    ...
    agriculture_land_bin_acc  = accuracy(agriculture_land_bin, yb['agriculture_land_bin'], topk=(1,2))
    ...
    accs = t2s([wealth_acc, water_src_acc, toilet_type_acc, roof_acc, cooking_fuel_acc, drought_acc, pop_density_acc, livestock_bin_acc, agriculture_land_bin_acc])
...
def fit(epochs, model, opt, scheduler, criterion, train_dl, valid_dl):
    for epoch in range(epochs):
        ...
        model.eval()
        with torch.no_grad():
            losses, accs, nums = .0, [[.0,.0] for i in range(9)], .0
...
def main():
    ...
    # cycle_fn = partial(optim.lr_scheduler.CyclicLR,
    #                    mode='triangular2',
    #                    base_lr=lr/10,
    #                    max_lr=lr,
    ...
    print('\n\nPhase 1, 128, 64', learn.wrapper.grads)
    learn.fit(3, 0.1, db, opt_fn, step_fn)
    torch.save(learn.wrapper.model.state_dict(), 'r_phase1.pt')

    # Freeze: Partial(0.7), 3 epochs
    learn.wrapper.partial_freeze_features(0.7)
    print('\n\nPhase 2, 128, 64', learn.wrapper.grads)
    learn.fit(1, 0.001, db, opt_fn, step_fn)
    learn.fit(2, 0.1, db, opt_fn, step_fn)
    torch.save(learn.wrapper.model.state_dict(), 'r_phase2.pt')

    # Freee: None, 3 Epochs
    learn.wrapper.freeze_features(False)
    print('\n\nPhase 3, 128, 64', learn.wrapper.grads)
    learn.fit(1, 0.001, db, opt_fn, step_fn)
    learn.fit(2, 0.1, db, opt_fn, step_fn)
    torch.save(learn.wrapper.model.state_dict(), 'r_phase3.pt')
    ...
    learn.fit(1, 0.001, db, opt_fn, step_fn)
    learn.fit(3, 0.1, db, opt_fn, step_fn)
    torch.save(learn.wrapper.model.state_dict(), 'r_phase4.pt')

    # IMG_SZ = 299, BS: 32
    db = get_data(img_sz=299, bs=32)

    print('\n\nPhase 4, 299, 32', learn.wrapper.grads)
    learn.fit(1, 0.001, db, opt_fn, step_fn)
    learn.fit(3, 0.1, db, opt_fn, step_fn)
    torch.save(learn.wrapper.model.state_dict(), 'r_phase5.pt')
